...
-- Creates separate schemas for MedusaJS, Beauty Shop, and Payload CMS
...
-- Move MedusaJS tables to medusa schema
-- Note: This will be done by MedusaJS migrations, but we ensure the schema exists

-- Create Beauty Shop specific tables
CREATE TABLE IF NOT EXISTS beauty_shop.user_profiles (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  clerk_user_id TEXT NOT NULL UNIQUE,
  customer_id UUID, -- Will reference medusa.customer(id) after MedusaJS setup
  phone VARCHAR(20),
...
-- Note: These will be updated once Clerk is properly configured
